describe "JSON grammar", ->
  ...
  beforeEach ->
    waitsForPromise ->
      atom.packages.activatePackage('lenientjs')

    runs ->
      grammar = atom.grammars.grammarForScopeName('source.json.lenient')

  it "parses the grammar", ->
    expect(grammar).toBeDefined()
    expect(grammar.scopeName).toBe 'source.json.lenient'

  it "tokenizes arrays", ->
    baseScopes = ['source.json.lenient', 'meta.structure.array.json']
  ...
  it "tokenizes objects", ->
    baseScopes = ['source.json.lenient', 'meta.structure.dictionary.json']
  ...
  it "tokenizes bare keys", ->
    ...
    expect(tokens[0]).toEqual value: '{', scopes: [baseScopes..., 'punctuation.definition.dictionary.begin.json']
    expect(tokens[1]).toEqual value: 'a', scopes: keyScopes
    expect(tokens[2]).toEqual value: ':', scopes: keyValueSeparatorScopes
    expect(tokens[3]).toEqual value: ' ', scopes: valueScopes
    expect(tokens[4]).toEqual value: '1', scopes: [valueScopes..., 'constant.numeric.json']
    expect(tokens[5]).toEqual value: ',', scopes: pairSeparatorScopes
    expect(tokens[6]).toEqual value: ' ', scopes: baseScopes
    expect(tokens[7]).toEqual value: 'b', scopes: keyScopes
    expect(tokens[8]).toEqual value: ':', scopes: keyValueSeparatorScopes
    expect(tokens[9]).toEqual value: ' ', scopes: valueScopes
    expect(tokens[10]).toEqual value: '2', scopes: [valueScopes..., 'constant.numeric.json']
    expect(tokens[11]).toEqual value: '}', scopes: [baseScopes..., 'punctuation.definition.dictionary.end.json']

  fit "tokenizes bare keys across lines", ->
    baseScopes = ['source.json.lenient', 'meta.structure.dictionary.json']
    keyScopes = [baseScopes..., 'meta.structure.dictionary.key.json', 'string.quoted.double.json']
    valueScopes = [baseScopes..., 'meta.structure.dictionary.value.json']
    keyValueSeparatorScopes = [valueScopes..., 'punctuation.separator.dictionary.key-value.json']
    ...
    expect(tokens[6]).toEqual value: 'b', scopes: keyScopes
    expect(tokens[7]).toEqual value: ':', scopes: keyValueSeparatorScopes
    expect(tokens[8]).toEqual value: ' ', scopes: valueScopes
    expect(tokens[9]).toEqual value: '2', scopes: [valueScopes..., 'constant.numeric.json']
    expect(tokens[10]).toEqual value: '}', scopes: [baseScopes..., 'punctuation.definition.dictionary.end.json']
